<template>
    <div>
        <el-row class="search-container center-search" justify="center" style="margin-top: 2rem;">
            <el-col :span="8">
                <el-input v-model="searchTerm" placeholder="搜索宝可梦..." size="large">
                    <template #append>
                        <el-button slot="append" icon="el-icon-search" @click="filteredPokemonsBySearch"
                            style="background-color: #ADD8E6; border-color: #ADD8E6;"></el-button>
                    </template>
                </el-input>
            </el-col>
        </el-row>
        <div class="type-selector">
            <el-button v-for="typeName in uniqueTypes" :key="typeName"
                :class="['type-button', typeName, { 'selected': selectedTypes.includes(typeName) }]"
                :style="{ backgroundColor: typecolors[typeName], width: '100px' }"
                @click="toggleTypeSelection(typeName)">
                {{ getTranslatedTypeName(typeName) }}
            </el-button>
        </div>
        <div id="back-to-top" class="back-to-top" v-show="showBackToTop" @click="scrollToTop">
            <i class="el-icon-arrow-up"></i>
            <span class="back-to-top-text">返回顶部</span>
        </div>
        <div class="aj">
            <div class="ak" v-for="itemg in filteredPokemonsBySearch" :key="itemg.id"
                @click="$router.push(`./${itemg.id}`)">
                <div class="al" :style="{ backgroundColor: colors[itemg.types[0].type.name] }">
                    <div class="am">
                        <img class="an" :src="itemg.sprites.front_default">
                    </div>
                    <div class="ao">编号：{{ itemg.id.toString().padStart(5, '0') }}</div>
                    <h3 class="ap">{{ getTranslatedName(itemg.name) }}</h3>
                    <div class="aq">
                        <div class="as" v-for="itemh in itemg.types" :key="itemh.id">
                            <div class="ar" :style="{ backgroundColor: typecolors[itemh.type.name] }">{{
                                getTranslatedTypeName(itemh.type.name) }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            searchTerm: '', // 用户输入的搜索词
            selectedTypes: [], // 用来存储选中的类型
            uniqueType: [], // 存储所有唯一的类型
            showBackToTop: false
        }
    },
    created() {
        this.fetchData()
        this.fetchUniqueTypes()// 创建组件时获取数据
    },
    mounted() {
    ...
    computed: {
        filteredPokemonsBySearch() {
            ...
            if (this.searchTerm) {
                // 如果有搜索词，则先根据搜索词过滤
                const searchTermAsNumber = parseInt(this.searchTerm, 10)

                if (!isNaN(searchTermAsNumber)) {
                    filteredPokemons = this.pokemons.filter(pokemon => {
                        return pokemon.id.toString().includes(searchTermAsNumber.toString())
                    })
                } else {
                    const term = this.searchTerm.toLowerCase()
                    const translatedTerm = Object.keys(this.pokename).find(key => this.pokename[key] === term)
                    const searchTerms = [term, translatedTerm].filter(Boolean)

                    filteredPokemons = this.pokemons.filter(pokemon => {
                        return searchTerms.some(searchTerm =>
                            pokemon.name.toLowerCase().includes(searchTerm) ||
                            this.getTranslatedName(pokemon.name).toLowerCase().includes(searchTerm)
                        )
                    })
                }
            }
            ...
            if (this.selectedTypes.length > 0) {
                filteredPokemons = filteredPokemons.filter(pokemon => {
                    // 当选择了一个类型时，只需要匹配其中一个类型即可
                    if (this.selectedTypes.length === 1) {
                        return pokemon.types.some(pokemonType => pokemonType.type.name === this.selectedTypes[0])
                    } else {
                        // 当选择了两个类型时，需要匹配所有选中的类型
    ...
    methods: {
        async fetchData() {
            ...
            const fetchRange = async (start, end) => {
                ...
                return responses.filter(response => response.data.sprites.front_default) // 过滤掉 front_default 为空的对象
            }
            try {
                // 获取1到1025的宝可梦数据
                const firstBatch = await fetchRange(1, 1025)
                this.pokemons.push(...firstBatch.map(response => response.data))

                // 获取10001到10277的宝可梦数据（假设这些ID是有效的）
                const secondBatch = await fetchRange(10001, 10277)
                this.pokemons.push(...secondBatch.map(response => response.data))
            } catch (error) {
        ...
        toggleTypeSelection(typeName) {
            if (this.selectedTypes.includes(typeName)) {
                // 如果已选中，则移除
                this.selectedTypes.splice(this.selectedTypes.indexOf(typeName), 1)
            } else {
                // 如果未选中，则添加
        ...
        // 获取所有唯一的类型
        fetchUniqueTypes() {
            this.uniqueType = [...new Set(this.pokemons.flatMap(pokemon => pokemon.types.map(type => type.type.name)))]
        },
        getTranslatedName(name) {
            return this.pokename[name] || name
        },
        getTranslatedTypeName(name) {
...
.center-search .el-input {
    width: 100%;
    /* 默认宽度为 100%，但会被 el-col 控制 */
    max-width: 400px;
    /* 最大宽度限制 */
...
.center-search .el-input-group__append .el-button {
...
/* 确保按钮颜色正确 */
.center-search .el-button--default {
...
/* 示例类型颜色 */
.type-selector .type-button.bug {
    background-color: #a8b820;
}

.type-selector .type-button.dragon {
    background-color: #7038f8;
}

.type-selector .type-button.electric {
    background-color: #f8d030;
}

.type-selector .type-button.fairy {
    background-color: #ee99ac;
}

.type-selector .type-button.fighting {
    background-color: #c03028;
}

.type-selector .type-button.fire {
    background-color: #f08030;
}

.type-selector .type-button.flying {
    background-color: #a890f0;
}

.type-selector .type-button.ghost {
    background-color: #705898;
}

.type-selector .type-button.grass {
    background-color: #78c850;
}

.type-selector .type-button.ground {
    background-color: #e0c068;
}

.type-selector .type-button.ice {
    background-color: #98d8d8;
}

.type-selector .type-button.normal {
    background-color: #a8a878;
}

.type-selector .type-button.psychic {
    background-color: #f85888;
}

.type-selector .type-button.poison {
    background-color: #a040a0;
}

.type-selector .type-button.rock {
    background-color: #b8a038;
}

.type-selector .type-button.steel {
    background-color: #b8b8d0;
}

.type-selector .type-button.water {

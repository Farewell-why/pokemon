<template>
    <div class="de">
        <div class="da">
            <div class="section-header">选择精灵</div>
            <el-select v-model="value" filterable placeholder="请选择">
                <el-option v-for="itemm in fairy" :key="itemm.id" :label="getChineseName(itemm.name)"
                    :value="itemm.name">
                </el-option>
            </el-select>
            <el-switch v-model="isShiny" active-text="闪光" inactive-text="普通"></el-switch>
            <div class="section-header">特性</div>
            <el-select v-if="value" v-model="abilityValue" placeholder="请选择特性">
                <el-option v-for="ability in abilities" :key="ability.ability.name" :label="ability.ability.name"
                    :value="ability.ability.name">
                </el-option>
            </el-select>
            <div v-if="value">
                <div class="types-container">
                    <div v-for="types in poke.types" :key="types.slot" class="type-badge"
                        :style="{ backgroundColor: fairytypecolors[types.type.name] }"> {{
                            getTypeChineseName(types.type.name) }}</div>
                </div>
            </div>
            <div class="section-header">性格</div>
            <el-select v-if="value" v-model="natureValue" placeholder="请选择性格">
                <el-option v-for="nature in natures" :key="nature.name" :label="`${nature.name} (${nature.effect})`"
                    :value="nature.name">
                </el-option>
            </el-select>
            <img v-if="value" class="pokemon-image" :src="selectedSprite" alt="宝可梦图片">
        </div>
        <div class="db">
            <!-- 添加输入框 -->
            <div class="section-header">精灵当前的等级</div>
            <div class="input-group">
                <el-input-number v-model="level" :min="1" :max="100" label="精灵等级">
                </el-input-number>
                <el-button size="small" @click="level = 1">min</el-button>
                <el-button size="small" @click="level = 100">max</el-button>
            </div>
            <div class="section-header">个体值 (IVs)</div>
            <div class="info-text">个体值是随机分配的，不可改变，影响各基础能力值。</div>

            <div class="label-group">
                <label>生命个体值:</label>
                <el-input-number v-model="individualValues.hp" :min="0" :max="31" label="生命个体值">
                </el-input-number>
                <el-button size="small" @click="individualValues.hp = 0">min</el-button>
                <el-button size="small" @click="individualValues.hp = 31">max</el-button>
            </div>

            <div class="label-group">
                <label>攻击个体值:</label>
                <el-input-number v-model="individualValues.attack" :min="0" :max="31" label="攻击个体值">
                </el-input-number>
                <el-button size="small" @click="individualValues.attack = 0">min</el-button>
                <el-button size="small" @click="individualValues.attack = 31">max</el-button>
            </div>

            <div class="label-group">
                <label>防御个体值:</label>
                <el-input-number v-model="individualValues.defense" :min="0" :max="31" label="防御个体值">
                </el-input-number>
                <el-button size="small" @click="individualValues.defense = 0">min</el-button>
                <el-button size="small" @click="individualValues.defense = 31">max</el-button>
            </div>

            <div class="label-group">
                <label>特攻个体值:</label>
                <el-input-number v-model="individualValues.specialAttack" :min="0" :max="31" label="特攻个体值">
                </el-input-number>
                <el-button size="small" @click="individualValues.specialAttack = 0">min</el-button>
                <el-button size="small" @click="individualValues.specialAttack = 31">max</el-button>
            </div>

            <div class="label-group">
                <label>特防个体值:</label>
                <el-input-number v-model="individualValues.specialDefense" :min="0" :max="31" label="特防个体值">
                </el-input-number>
                <el-button size="small" @click="individualValues.specialDefense = 0">min</el-button>
                <el-button size="small" @click="individualValues.specialDefense = 31">max</el-button>
            </div>

            <div class="label-group">
                <label>速度个体值:</label>
                <el-input-number v-model="individualValues.speed" :min="0" :max="31" label="速度个体值">
                </el-input-number>
                <el-button size="small" @click="individualValues.speed = 0">min</el-button>
                <el-button size="small" @click="individualValues.speed = 31">max</el-button>
            </div>

        </div>
        <div class="dc">
            <div class="section-header">努力值 (EVs)</div>
            <div class="info-text">努力值是通过训练获得的，影响各基础能力值。</div>

            <div class="label-group ev-group">
                <label>生命努力值:</label>
                <el-input-number v-model="effortValues.hp" :min="0" :max="255" label="生命努力值"
                    :class="{ 'error': effortSum > 510 }">
                </el-input-number>
                <el-button size="small" @click="effortValues.hp = 0">min</el-button>
                <el-button size="small" @click="effortValues.hp = 255">max</el-button>
            </div>

            <div class="label-group ev-group">
                <label>攻击努力值:</label>
                <el-input-number v-model="effortValues.attack" :min="0" :max="255" label="攻击努力值"
                    :class="{ 'error': effortSum > 510 }">
                </el-input-number>
                <el-button size="small" @click="effortValues.attack = 0">min</el-button>
                <el-button size="small" @click="effortValues.attack = 255">max</el-button>
            </div>

            <div class="label-group ev-group">
                <label>防御努力值:</label>
                <el-input-number v-model="effortValues.defense" :min="0" :max="255" label="防御努力值"
                    :class="{ 'error': effortSum > 510 }">
                </el-input-number>
                <el-button size="small" @click="effortValues.defense = 0">min</el-button>
                <el-button size="small" @click="effortValues.defense = 255">max</el-button>
            </div>

            <div class="label-group ev-group">
                <label>特攻努力值:</label>
                <el-input-number v-model="effortValues.specialAttack" :min="0" :max="255" label="特攻努力值"
                    :class="{ 'error': effortSum > 510 }">
                </el-input-number>
                <el-button size="small" @click="effortValues.specialAttack = 0">min</el-button>
                <el-button size="small" @click="effortValues.specialAttack = 255">max</el-button>
            </div>

            <div class="label-group ev-group">
                <label>特防努力值:</label>
                <el-input-number v-model="effortValues.specialDefense" :min="0" :max="255" label="特防努力值"
                    :class="{ 'error': effortSum > 510 }">
                </el-input-number>
                <el-button size="small" @click="effortValues.specialDefense = 0">min</el-button>
                <el-button size="small" @click="effortValues.specialDefense = 255">max</el-button>
            </div>

            <div class="label-group ev-group">
                <label>速度努力值:</label>
                <el-input-number v-model="effortValues.speed" :min="0" :max="255" label="速度努力值"
                    :class="{ 'error': effortSum > 510 }">
                </el-input-number>
                <el-button size="small" @click="effortValues.speed = 0">min</el-button>
                <el-button size="small" @click="effortValues.speed = 255">max</el-button>
            </div>

            <div v-if="effortSum > 510" class="warning-message">
                注意：宝可梦的努力值总和不应超过 510!
            </div>
        </div>
        <div class="dd">
            <div ref="radar" class="radar-chart" style="width: 400px; height: 400px;"></div>
        </div>
        <div class="df">
            <div v-if="value" class="section-stats">
                <div class="section-header">能力值:</div>
                <ul class="stats-list">
                    <li>生命: {{ radarData.series[0].data[0].value[0] }}</li>
                    <li>攻击: {{ radarData.series[0].data[0].value[1] }}</li>
                    <li>防御: {{ radarData.series[0].data[0].value[2] }}</li>
                    <li>特攻: {{ radarData.series[0].data[0].value[3] }}</li>
                    <li>特防: {{ radarData.series[0].data[0].value[4] }}</li>
                    <li>速度: {{ radarData.series[0].data[0].value[5] }}</li>
                    <li>总和: {{ radarData.series[0].data[0].value.reduce((acc, val) => acc + val, 0) }}</li>
                </ul>
            </div>
        </div>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            natures: [ // 性格及其效果
                { name: '勤奋', effect: '无变化', modifiers: [1, 1, 1, 1, 1, 1] },
                { name: '怕寂寞', effect: '+攻击，-防御', modifiers: [1, 1.1, 0.9, 1, 1, 1] },
                { name: '固执', effect: '+攻击，-特攻', modifiers: [1, 1.1, 1, 0.9, 1, 1] },
                { name: '顽皮', effect: '+攻击，-特防', modifiers: [1, 1.1, 1, 1, 0.9, 1] },
                { name: '勇敢', effect: '+攻击，-速度', modifiers: [1, 1.1, 1, 1, 1, 0.9] },
                { name: '坦率', effect: '无变化', modifiers: [1, 1, 1, 1, 1, 1] },
                { name: '大胆', effect: '+防御，-攻击', modifiers: [1, 0.9, 1.1, 1, 1, 1] },
                { name: '淘气', effect: '+防御，-特攻', modifiers: [1, 1, 1.1, 0.9, 1, 1] },
                { name: '乐天', effect: '+防御，-特防', modifiers: [1, 1, 1.1, 1, 0.9, 1] },
                { name: '悠闲', effect: '+防御，-速度', modifiers: [1, 1, 1.1, 1, 1, 0.9] },
                { name: '害羞', effect: '无变化', modifiers: [1, 1, 1, 1, 1, 1] },
                { name: '内敛', effect: '+特攻，-攻击', modifiers: [1, 0.9, 1, 1.1, 1, 1] },
                { name: '慢吞吞', effect: '+特攻，-防御', modifiers: [1, 1, 0.9, 1.1, 1, 1] },
                { name: '马虎', effect: '+特攻，-特防', modifiers: [1, 1, 1, 1.1, 0.9, 1] },
                { name: '冷静', effect: '+特攻，-速度', modifiers: [1, 1, 1, 1.1, 1, 0.9] },
                { name: '浮躁', effect: '无变化', modifiers: [1, 1, 1, 1, 1, 1] },
                { name: '温和', effect: '+特防，-攻击', modifiers: [1, 0.9, 1, 1, 1.1, 1] },
                { name: '温顺', effect: '+特防，-防御', modifiers: [1, 1, 0.9, 1, 1.1, 1] },
                { name: '慎重', effect: '+特防，-特攻', modifiers: [1, 1, 1, 0.9, 1.1, 1] },
                { name: '自大', effect: '+特防，-速度', modifiers: [1, 1, 1, 1, 1.1, 0.9] },
                { name: '认真', effect: '无变化', modifiers: [1, 1, 1, 1, 1, 1] },
                { name: '胆小', effect: '+速度，-攻击', modifiers: [1, 0.9, 1, 1, 1, 1.1] },
                { name: '急躁', effect: '+速度，-防御', modifiers: [1, 1, 0.9, 1, 1, 1.1] },
                { name: '爽朗', effect: '+速度，-特攻', modifiers: [1, 1, 1, 0.9, 1, 1.1] },
                { name: '天真', effect: '+速度，-特防', modifiers: [1, 1, 1, 1, 0.9, 1.1] }
            ],
            natureValue: null,
            level: 100, // 精灵等级
            effortValues: {
    ...
    computed: {
        effortSum() {
            return Object.values(this.effortValues).reduce((acc, value) => acc + value, 0)
        }
    },
    async created() {
        const fetchRange = async (start, end) => {
            ...
            return responses.filter(response => response.data.sprites.front_default)
        }
        try {
            const firstBatch = await fetchRange(1, 1025)
            this.fairy.push(...firstBatch.map(response => response.data))

            const secondBatch = await fetchRange(10001, 10277)
            this.fairy.push(...secondBatch.map(response => response.data))
        } catch (error) {
    ...
    watch: {
        value(newValue, oldValue) {
            if (newValue) {
                const selectedItem = this.fairy.find(itemm => itemm.name === newValue)
                if (selectedItem) {
                    this.poke = selectedItem
                    this.selectedSprite = this.isShiny ? selectedItem.sprites.front_shiny : selectedItem.sprites.front_default
                    this.abilities = selectedItem.abilities
                    this.abilityValue = this.abilities.length > 0 ? this.abilities[0].ability.name : null
                ...
                this.poke = null
                this.natureValue = null
                this.radarData.series[0].data[0].value = []
                this.$nextTick(() => {
                    this.initRadarChart()
                })
            }
        ...
        isShiny(newVal) {
            if (this.value && newVal !== undefined) {
                this.selectedSprite = newVal ? this.poke.sprites.front_shiny : this.poke.sprites.front_default
            }
        },
        level() {
            this.updateRadarChart()
    ...
    methods: {
        getChineseName(name) {
            return this.fairyname[name] || name
        },
        getTypeChineseName(name) {
            return this.fairytypenames[name] || name
        },
        calculateStat(statIndex, baseStat, natureModifiers) {
            let stat = 0
            if (statIndex === 0) { // HP
                stat = Math.floor(((baseStat * 2 + this.individualValues.hp + this.effortValues.hp / 4) * this.level / 100) + 10 + this.level)
            } else { // 其他五项
                const individualKey = ['hp', 'attack', 'defense', 'specialAttack', 'specialDefense', 'speed'][statIndex]
                const effortKey = ['hp', 'attack', 'defense', 'specialAttack', 'specialDefense', 'speed'][statIndex]

                stat = Math.floor((((baseStat * 2 + this.individualValues[individualKey]) + this.effortValues[effortKey] / 4) * this.level / 100) + 5) * natureModifiers[statIndex]
            }
            return Math.round(stat)
        },
        updateRadarChart() {
            if (this.poke && this.natureValue) {
                const nature = this.natures.find(n => n.name === this.natureValue)
                const natureModifiers = nature.modifiers
                const stats = ['hp', 'attack', 'defense', 'specialAttack', 'specialDefense', 'speed'].map((stat, index) => {
                    return this.calculateStat(index, this.poke.stats[index].base_stat, natureModifiers)
                })

                this.radarData.series[0].data[0].value = stats
                this.radarData.series[0].data[0].name = `${this.getChineseName(this.value)} (${this.value})`

                this.radarData.series[0].data[0].label = {
                    normal: {
                        show: true,
                        position: 'inside',
                        formatter: '{c}', // 显示宝可梦名称和数值
                        textStyle: {
                            color: '#000',
                            fontSize: 12
                        }
                ...
                this.radarData.series[0].data[0].labelLine = {
                    normal: {
                        show: true,
                        length: 10,
                        length2: 10
                ...
                this.radarData.series[0].itemStyle = {
                    normal: {
                        color: '#FF6600',
                        shadowBlur: 10,
                ...
                this.radarData.series[0].areaStyle = {
                    normal: {
...
.ev-group.error .el-input__inner {
    border-color: red !important;
}
